...
$PI.onConnected(jsn => {
    ...
    Object.entries(jsn.actionInfo.payload.settings).forEach(([key, value]) => {
        ...
        el.value = value;
        // If el is text input, set value
        if (el && el.tagName === 'INPUT') {
            el.value = value;
        } else if (el && el.tagName === 'SELECT') {
            // If current value is not in options, add it
            if (!el.options[el.selectedIndex] || el.options[el.selectedIndex].value !== value) {
    ...
    $PI.onSendToPropertyInspector(actionUUID, jsn => {
        ...
        updateSelect(document.querySelector('#receiverName'), jsn.payload.receivers);
    });
});
...
const getReceivers = (type) => {
    $PI.sendToPlugin({getReceivers: type});
}
...
function updateSelect(el, options) {
    console.log('updateSelect', el, options);
    const v = el.value;
    // Add current value to options if not already in there
    if(!options.includes(v)) {
        // Prepend
        options.unshift(v);
    }
    // Append an <option> for each receiver name
    el.innerHTML = options.map(e => `<option value="${e}">${e}</option>`).join('');
    if (v) {
        el.value = v;
    }
}
